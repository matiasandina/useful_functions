...
%  UPDATE %% 
%    
%       'DisplayCoords' : TRUE or FALSE specifying whether to show the
%                         coords on the top left. Default is false 
%       
%
...
addParamValue(p, 'ConnectPoints', true, @(x) validateattributes(x, ...
    {'logical'}, {'scalar'}));
addParamValue(p, 'DisplayCoords', false, @(x) validateattributes(x, ...
    {'logical'}, {'scalar'}));
...
DisplayCoords = p.Results.DisplayCoords;
...
%--------------------------------------------------------------------------
    function updatePoints(clickType)
        ...
        % To close polygon and exit
        if(clickType == 3)
            
            if connectpoints

             % Make the last point to be equal to the first
             x = [x(:) ; x(1)];   
             y = [y(:) ; y(1)];
            end
